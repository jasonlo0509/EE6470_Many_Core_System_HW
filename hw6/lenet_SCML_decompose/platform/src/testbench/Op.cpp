...
void pool2d(TVMValue stack_value, int arg_num) {
  ...
  if (print_shape) {
    ...
    std::cout << "stride: " << stride << "\n";
    std::cout << "\t";
    std::cout << "each decomposed ch size: " << data_shape.c/4 << "\n";
  }
  ...
  pool_config.data_cube_in_channel  = data_shape.c/4; // decompose along channel
  ...
  pool_config.data_cube_out_channel = output_shape.c/4; // decompose along channel
  ...
  if (data_shape.w == 12){
    std::cout << "[wait for 12]\n";
    wait(18913,SC_NS);
  }
  else if (data_shape.w == 4){
    std::cout << "[wait for 4]\n";
    wait(5900,SC_NS);
  }
  else {
